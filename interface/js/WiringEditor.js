...
WireIt.WiringEditor.prototype = {
 ...
 setOptions: function(options) {
    ...
    for(var i = 0 ; i < this.modules.length ; i++) {
	 ...
	 this.adapter = options.adapter || WireIt.WiringEditor.adapters.JQuery;
 ...
 saveModule: function() {
    ...
    if(value.project === "") {
       this.alert("Please name the project.");
       return;
    }
 ...
 onRun: function() {
   ...
   this.adapter.runCode(value.working);
 },
    ...
    if(lang.isArray(this.pipes)) {
       for(var i = 0 ; i < this.pipes.length ; i++) {
          ...
          if(!filter || filter === "" || module.name.match(new RegExp(filter,"i")) ) {
				/* FIXME: Is this the right way to improve the interface for loading projects? */
	          list.appendChild( WireIt.cn('li',null,{cursor: 'pointer'}, module.name + "_" + module.project) );
			}
       }
    }
 ...
 getPipeByName: function(name) {
    ...
    for(i = 0 ; i < n ; i++) {
       if(this.pipes[i].name + "_" + this.pipes[i].project == name) {
